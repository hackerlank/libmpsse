 ...
 * On failure, mpsse->open will be set to 0.
 */
struct mpsse_context *Open(int vid, int pid, enum modes mode, int freq, int endianess, int interface1, const char *description, const char *serial)
{
	return OpenIndex(vid, pid, mode, freq, endianess, interface1, description, serial, 0);
}
 ...
 * On failure, mpsse->open will be set to 0.
 */
struct mpsse_context *OpenIndex(int vid, int pid, enum modes mode, int freq, int endianess, int interface1, const char *description, const char *serial, int index)
{
	int status = 0;
	struct mpsse_context *mpsse = NULL;
	...
	if(mpsse)
	{
		...
		if(ftdi_init(&mpsse->ftdi) == 0)
		{
			...
			ftdi_set_interface(&mpsse->ftdi, interface1);
