...
static unsigned int _ftdi_determine_max_packet_size(struct ftdi_context *ftdi, libusb_device *dev)
{
    ...
    {
        if (ftdi->interface1 < config0->bNumInterfaces)
        {
            struct libusb_interface interface = config0->interface[ftdi->interface1];
...
int ftdi_usb_open_dev(struct ftdi_context *ftdi, libusb_device *dev)
{
    ...
    {
        if (libusb_detach_kernel_driver(ftdi->usb_dev, ftdi->interface1) !=0)
            detach_errno = errno;
    }
    ...
    if (libusb_claim_interface(ftdi->usb_dev, ftdi->interface1) < 0)
    {
        ftdi_usb_close_internal (ftdi);
        if (detach_errno == EPERM)
        {
...
int ftdi_usb_close(struct ftdi_context *ftdi)
{
    ...
    if (ftdi->usb_dev != NULL)
        if (libusb_release_interface(ftdi->usb_dev, ftdi->interface1) < 0)
            rtn = -1;
...
struct ftdi_transfer_control *ftdi_write_data_submit(struct ftdi_context *ftdi, unsigned char *buf, int size)
{
    ...
    libusb_fill_bulk_transfer(transfer, ftdi->usb_dev, ftdi->in_ep, buf,
                              write_size, (libusb_transfer_cb_fn)ftdi_write_data_cb, tc,
...
struct ftdi_transfer_control *ftdi_read_data_submit(struct ftdi_context *ftdi, unsigned char *buf, int size)
{
    ...
    libusb_fill_bulk_transfer(transfer, ftdi->usb_dev, ftdi->out_ep, ftdi->readbuffer, ftdi->readbuffer_chunksize, (libusb_transfer_cb_fn)ftdi_read_data_cb, tc, ftdi->usb_read_timeout);
    transfer->type = LIBUSB_TRANSFER_TYPE_BULK;
